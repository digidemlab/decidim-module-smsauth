  ...
  module Smsauth
    class OmniauthController < ::Decidim::Devise::OmniauthRegistrationsController
      ...
      def send_message
        ...
        ::Decidim::Smsauth::SendVerificationCode.call(@form, organization: current_organization) do
          on(:ok) do |result, notice|
            generate_sessions!(result)
            if notice.present?
              flash[:alert] = I18n.t(".server_busy", scope: "decidim.smsauth.omniauth.send_message", phone: formatted_phone_number(@form))
            else
          ...
          on(:invalid) do
            flash[:alert] = I18n.t(".generic_error", scope: "decidim.smsauth.omniauth.send_message.error")
            render action: "new"
          end
        end
      end
      ...
      def access_code_validation
        ...
        VerifyAccessCode.call(@form) do
          ...
          on(:invalid) do
            flash.now[:alert] = I18n.t(".access_code_error", scope: "decidim.smsauth.omniauth.authenticate_user")
      ...
      def unique_id(user)
        Digest::MD5.hexdigest(
          "#{::Decidim::Smsauth.country_code}-#{user.phone_number}-#{Rails.application.secrets.secret_key_base}"
        )
      end
      ...
      def set_flash_message_for_resend
        if code_expired?
          flash[:alert] = I18n.t(".expired", scope: "decidim.smsauth.omniauth.send_message", phone: formatted_phone_number(@form))
        else
