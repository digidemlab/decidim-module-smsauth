    ...
    module Verifications
      class AuthorizationsController < Decidim::Verifications::ApplicationController
        ...
        def school_info
          ...
          @form = form(::Decidim::Smsauth::SchoolMetadataForm).instance
        end

        def school_validation
          enforce_permission_to(:update, :authorization, authorization:)

          @form = form(::Decidim::Smsauth::SchoolMetadataForm).from_params(params)
        ...
        def access_code
          ...
          @form = form(::Decidim::Smsauth::AccessCodeForm).instance
        end

        def access_code_validation
          @form = form(::Decidim::Smsauth::AccessCodeForm).from_params(params.merge({ current_locale:, organization: current_organization }))
        ...
        def handle_redirect
          if authorization.metadata["school"].nil?
            flash[:notice] = I18n.t(".school_info", scope: "decidim.smsauth.verification.authorizations.school_info")
        ...
        def flash_message_for_resend(last_request)
          if long_ago?(last_request)
            flash[:alert] = I18n.t(".expired", scope: "decidim.smsauth.omniauth.send_message")
          else
            flash[:notice] = t("authorizations.create.success", scope: "decidim.verifications.sms")
          end
        end
