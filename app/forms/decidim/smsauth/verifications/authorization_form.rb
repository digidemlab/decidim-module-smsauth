      ...
      class AuthorizationForm < Decidim::AuthorizationHandler
        ...
        def unique_id
          Digest::MD5.hexdigest(
            "#{::Decidim::Smsauth.country_code}-#{phone_number}-#{Rails.application.secrets.secret_key_base}"
          )
        end
        ...
        def sms_sending_error(error_code)
          ...
          when :destination_whitelist
            I18n.t(".destination_whitelist", scope: "decidim.smsauth.omniauth.send_message.error")
          when :destination_blacklist
            I18n.t(".destination_blacklist", scope: "decidim.smsauth.omniauth.send_message.error")
          when :server_error
            I18n.t(".server_error", scope: "decidim.smsauth.omniauth.send_message.error")
          else
