...
module Decidim
  module Smsauth
    class VerifyAccessCode < Decidim::Command
      ...
      def generate_email
        "smsauth-#{generate_token("#{prefix}_#{form.access_code}")}@#{email_domain}"
      end

      def email_domain
        Decidim::Smsauth.auto_email_domain || form.organization.host
      end
      ...
      def access_code_instance(code)
        ...
        ::Decidim::Smsauth::SigninCode.find_by(code_hash:)
      end
      ...
      def create_code_session!
        ::Decidim::Smsauth::SigninCodeSession.create!(
          user: @user,
